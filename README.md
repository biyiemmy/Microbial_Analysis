...
![Heatmap Top 50 DEGs](images/heatmap_top50_DEGs.png)

**Description:** Heatmap summarizing the top 50 differentially expressed genes. Each column represents a sample, and each row represents a gene.
- **Red:** Upregulation
- **Blue:** Downregulation
- **Observation:** Control group (left columns) and treatment group (right columns) cluster together, showing consistent treatment effect on gene expression
...
- **Gene T00048 & T00034:** Strongly downregulated in treatment group
- **Interpretation:** These genes are key candidates for further study to uncover biological mechanisms behind treatment response

### Figure 3: Voom Mean-Variance Trend Plot
![Voom Plot](images/voom_plot.png)

**Description:** Mean-variance trend plot generated by the voom function from limma package, showing quality of RNA-seq normalization.
- **Each black dot:** Represents a single gene
- **X-axis:** Average log-expression
- **Y-axis:** Square root of standard deviation
- **Left side:** Low-expression genes with high variance (noisy)
- **Right side:** High-expression genes with low, consistent variance (stable)
- **Red trend line:** Smooth, decreasing curve indicating ideal voom transformation
- **Quality:** No abrupt breaks or weird patterns - excellent normalization achieved

### Figure 4: Principal Component Analysis (PCA) Plot
![PCA Plot](images/pca_plot.png)

**Description:** PCA showing sample clustering based on gene expression profiles.
- **PC1 (44.2% variance):** Primary axis separating treatment and control
- **PC2 (~20-25% variance):** Secondary biological variation
- **Blue points:** Control samples (clustered in top-left)
- **Red points:** Treatment samples (clustered in bottom-right)
- **Interpretation:** 
  - Treatment is the biggest biological factor influencing gene expression
  - Sodium propionate induces a strong, consistent, and reproducible transcriptomic response in MRSA
  - Excellent reproducibility across biological replicates

### Figure 5: Volcano Plot
![Volcano Plot](images/volcano_plot.png)

**Description:** Volcano plot showing statistical significance vs. biological significance of gene expression changes.
- **X-axis:** Log2 Fold Change (biological effect size)
- **Y-axis:** -Log10 Adjusted P-value (statistical significance)
- **Red points:** Differentially expressed genes (|log2FC| > 1, adj. p < 0.05)
- **Gray points:** Non-significant genes
- **Top right corner:** Most significant and most upregulated gene
- **Top left corner:** Most significant and most downregulated gene
- **Genes near origin (0,0):** Little change and not statistically significant
- **Vertical dashed lines:** Fold-change threshold (±1 log2 units)
- **Horizontal dashed line:** Statistical significance threshold (adj. p = 0.05)
